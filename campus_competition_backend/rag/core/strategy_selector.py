"""
通过提示词工程，调用LLM实现检索策略分类：直接检索、假设问题检索、子查询检索、回溯问题检索。
然后prompts模块根据检索策略来优化query
...
class StrategySelector:
    ...
    def call_dashscope(self, prompt):
        ...
        try:
            ...
            completion = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": RAGPrompts.strategy_system_prompt()},
    ...
    def select_strategy(self, query):
        ...
        print(f"选择检索策略的usr_prompt: {prompt}")
...
if __name__ == '__main__':
    ...
    strategy = selector.select_strategy(query="我有一个包含 1 条记录的数据集，想把它存储到 Milvus 中进行查询。可以吗？")
    # print(strategy)
