      ...
      >
        ...
        <el-table-column prop="request_method" label="请求方式" width="100">
          <template #default="{ row }">
            <el-tag :type="row.request_method === 'GET' ? 'info' : 'primary'">
              {{ row.request_method }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="request_url" label="请求URL" min-width="300" show-overflow-tooltip />
...
const batchDelete = async () => {
  ...
  try {
    ...
    await request.post('/operation-logs/batch-delete/', { ids })
...
const getList = async () => {
  ...
  try {
    ...
    ElMessage.error('获取操作日志失败：' + (err.message || '网络错误'))
    tableData.value = []
    total.value = 0
  } finally {
